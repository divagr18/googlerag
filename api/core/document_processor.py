...
def smart_word_doc_chunking(
    text: str, page_num: int, max_chunk_size: int = 1500, overlap_size: int = 150
) -> List[Tuple[str, int]]:
    """
    ...
    for i, sentence in enumerate(sentences):
        ...
        if (
            current_chunk_size + sentence_len > max_chunk_size
            ...
            overlap_sentences = (
                current_chunk_sentences[-1:] if current_chunk_sentences else []
            )
    ...
    print(
        f"Word doc chunking: Created {len(chunks)} chunks from {len(sentences)} sentences"
    )
    return chunks
...
async def download_with_aria2c(url: str, destination_path: str):
    ...
    def run_blocking_download():
        try:
            ...
            raise IOError(
                f"aria2c download failed with code {e.returncode}: {e.stderr}"
            )
...
async def process_document_stream_cached(
    url: str,
    document_iterator: Optional[AsyncIterator[bytes]] = None,
    use_cache: bool = True,
) -> List[Tuple[str, int]]:
    """
...
async def _extract_text_from_pdf_stream(
    ...
    tasks = [
        loop.run_in_executor(
            cpu_executor, _extract_page_text_from_bytes, (pdf_bytes, i)
        )
...
async def process_document_stream(
    url: str, document_iterator: AsyncIterator[bytes]
) -> List[Tuple[str, int]]:
    path = urlparse(url).path
    file_type = os.path.splitext(path)[1].lower() or ".txt"
    document_bytes_io = io.BytesIO(
        b"".join([chunk async for chunk in document_iterator])
    )
    loop = asyncio.get_running_loop()
    ...
    if file_type in [".txt", ".md", ".csv"]:
        return [(document_bytes_io.read().decode("utf-8", errors="ignore"), 1)]
    raise ValueError(f"Unsupported file type: '{file_type}'.")


def smart_paragraph_split(
    text: str, page_num: int, is_word_doc: bool = False
) -> List[Tuple[str, int]]:
    """
...
async def optimized_semantic_chunk_text(
    ...
    print(
        f"Successfully created {len(chunks)} semantic chunks with pre-computed embeddings."
    )
    return chunks, np.vstack(chunk_embs)


async def build_enhanced_retrieval_systems(
    chunks: List[Dict],
) -> Optional[DomainQueryExpander]:
    """
    ...
    try:
        ...
        query_expander = await loop.run_in_executor(
            cpu_executor,
            lambda: DomainQueryExpander(
                all_texts, min_term_freq=2, max_expansion_terms=4
            ),
        )
