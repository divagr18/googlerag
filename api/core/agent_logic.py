...
async def prepare_query_strategies_for_all_questions(
    questions: List[str],
) -> List[Dict]:
...
async def synthesize_answer_from_context(
    original_question: str, context: str, use_high_k: bool
) -> str:
...
async def synthesize_direct_answer(
    original_question: str, context: str, use_high_k: bool
) -> str:
    synthesis_prompt = f"""You are a world-class AI system specializing in analyzing and summarizing information from documents to answer user questions. Your response must be based exclusively on the provided evidence.
...
def _reciprocal_rank_fusion(
    search_results_lists: List[List[Tuple[Dict, float]]], k: int = 60
) -> List[Dict]:
...
async def rerank_chunks_batch(
    queries_and_chunks: List[Tuple[str, List[Dict]]],
) -> List[List[Dict]]:
    """
    ...
    print(
        f"🔄 Batch reranking {len(all_pairs)} total pairs across {len(queries_and_chunks)} queries"
    )
...
async def answer_questions_batch_orchestrator(
    ...
    print(
        f"🚀 Starting batched processing for {len(query_strategy_data_list)} questions"
    )
        ...
        for sub_query in sub_questions:
            search_tasks.append(
                knowledge_base.search(
                    sub_query, k=k_rerank_candidates, fusion_weights=(0.5, 0.5)
                )
            )
    ...
    for i, (original_question, sub_questions) in enumerate(
        zip(original_questions, all_sub_questions)
    ):
        ...
        if not fused_chunks:
            ...
            print(
                f"Retrieved and fused {len(fused_chunks)} candidates for: {original_question[:50]}..."
            )
    ...
    for i, (original_question, reranked_chunks) in enumerate(
        zip(original_questions, all_reranked_chunks)
    ):
...
async def answer_question_orchestrator(
    knowledge_base: RequestKnowledgeBase, query_strategy_data: Dict, use_high_k: bool
) -> Tuple[str, List[str]]:
    ...
    print(
        f"🧠 Decomposed into {len(sub_questions)} sub-queries for: {original_question[:50]}..."
    )
    ...
    final_answer = await synthesize_answer_from_context(
        original_question, aggregated_context, use_high_k=use_high_k
    )
